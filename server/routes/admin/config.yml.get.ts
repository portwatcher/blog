...
export default defineEventHandler((event) => {
  ...
  lines.push('media_library:')
  lines.push('  name: "blog-media-assets"')
  ...
  lines.push('        widget: "select"')
  lines.push('        default: "draft"')
  lines.push('        hint: "Drafts stay hidden from the public site. Use Private for password-protected posts, and Public when ready to publish."')
  lines.push('        options:')
  lines.push('          - { label: "Draft", value: "draft" }')
  lines.push('          - { label: "Public", value: "public" }')
  lines.push('          - { label: "Private", value: "private" }')
  lines.push('      - { label: "Legacy path", name: "legacyPath", widget: "hidden", required: false }')
  lines.push('      - { label: "Cover image", name: "cover", widget: "s3-image", required: false, hint: "Upload an image here to store it in S3 and save a structured media reference in frontmatter." }')
  lines.push('      - { label: "Featured video", name: "video", widget: "s3-video", required: false, hint: "Upload a video here to store it in S3 and save a structured media reference in frontmatter." }')
  lines.push('      - { label: "Body", name: "body", widget: "markdown", hint: "Rich text: Add Component -> S3 Image or S3 Video. Markdown mode: paste or drop an image/video to upload and insert it." }')
